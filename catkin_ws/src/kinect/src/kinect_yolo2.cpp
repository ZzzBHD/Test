...
void drawmainPred(int left, int top, int right, int bottom, Mat& frame)
{
    //Draw a rectangle displaying the bounding box
...
static int target_x=205//初始化中间位置
static int max_boxsize=0;
static int target_idx=0;
static int x_cache=100;
...
void postprocess(Mat& frame, const vector<Mat>& outs, float confThreshold, float nmsThreshold)
{
    ...
    for (size_t i = 0; i < indices.size(); ++i)
    {
        ...
        if(classIds[idx]==0)
        {
            max_boxsize=max(box.width*box.height,max_boxsize);
            if((box.x<=target_x+15)&&(box.x>=target_x-15))//目标位置跳变过滤,15需要按实际修改
                inperson_box.push_back(box);
            drawPred(classIds[idx], confidences[idx], box.x, box.y,box.x + box.width, box.y + box.height, frame);
        }
    }
    for(int i=0;i<inperson_box.size();i++)
    {
        if(x_cache>inperson_box[i].x)
        {
            x_cache=inperson_box[i].x;
            target_box=inperson_box[i];
        }
    }
    x_cache=100;
    inperson_box.clear();
    bounding_data.push_back (max_boxsize);
    bounding_data.push_back (target_box.x);
    drawmainPred(target_box.x, target_box.y,target_box.x + target_box.width, target_box.y + target_box.height, frame);
    bounding_flag=1;
}
